...
class Settings(object):
    ...
    def LoadSettings():
        ...
        Settings.AdditionalExtensionsToUpload = Settings.MakeListFromExtensionString(
            Settings.__GetDefault(configParser, "Settings", "AdditionalExtensionsToUpload", "bup, idx, ifo, srt, sub"))
        ...
        Settings.PtpImgApiKey = Settings.__GetDefault(configParser, "Settings", "PtpImgApiKey", "")
        Settings.OnSuccessfulUpload = Settings.__GetDefault(configParser, "Settings", "OnSuccessfulUpload", "",
                                                            raw=True)
        ...
        Settings.IgnoreReleaseTagAfterYear = Settings.MakeListOfListsFromString(
            Settings.__GetDefault(configParser, "Settings", "IgnoreReleaseTagAfterYear", ""))
        ...
        Settings.GreasemonkeyTorrentSenderPassword = Settings.__GetDefault(configParser, "Settings",
                                                                           "GreasemonkeyTorrentSenderPassword", "")
        Settings.OpenJobPageLinksInNewTab = Settings.__GetDefault(configParser, "Settings", "OpenJobPageLinksInNewTab",
                                                                  "0")
        Settings.OverrideScreenshots = int(
            Settings.__GetDefault(configParser, "Settings", "OverrideScreenshots", "0")) != 0
        Settings.ForceDirectorylessSingleFileTorrent = int(
            Settings.__GetDefault(configParser, "Settings", "MakeTorrentWithoutDirectory", "0")) != 0
        ...
        Settings.SkipDuplicateChecking = int(
            Settings.__GetDefault(configParser, "Settings", "SkipDuplicateChecking", "0")) != 0

        Settings.SizeLimitForAutoCreatedJobs = float(
            Settings.__GetDefault(configParser, "Settings", "SizeLimitForAutoCreatedJobs", "0")) * 1024 * 1024 * 1024
        ...
        Settings.StopIfCoverArtIsMissing = Settings.__GetDefault(configParser, "Settings", "StopIfCoverArtIsMissing",
                                                                 "")
        Settings.StopIfImdbRatingIsLessThan = Settings.__GetDefault(configParser, "Settings",
                                                                    "StopIfImdbRatingIsLessThan", "")
        Settings.StopIfImdbVoteCountIsLessThan = Settings.__GetDefault(configParser, "Settings",
                                                                       "StopIfImdbVoteCountIsLessThan", "")
    ...
    def __VerifyProgramPath(programName, arguments):
        ...
        try:
            ...
            MyGlobals.Logger.error("Execution of %s at '%s' caused an exception. Error message: '%s'." % (
